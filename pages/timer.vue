<template>
	<v-layout column justify-center align-center>
		...
		<v-btn v-if="pending" @click="restart" round color="primary"
			>再開</v-btn
		>
		...
		<v-btn
			...
			@click="reset"
			target="_blank"
			v-bind:href="getFinishedTweetURL()"
			rounded
			color="primary"
			>ツイートして戻る</v-btn
		>
...
export default {
	...
	methods: {
		...
		getStartTweetURL() {
			return (
				this.tweetBaseUrl + this.message + '&hashtags=やるぞタイマー'
			);
		},
		getFinishedTweetURL() {
			if (!this.tweet) {
			...
			if (this.failure) {
				...
				return this.getTweetURL(
					spent.toFixed(0) +
						'分しかやれなかった... 正直ごめんと思ってる...',
				);
			}
			if (this.lestMin <= 0) {
				return this.getTweetURL(
					'宣言通り' + this.min.toFixed(0) + '分やったぞ！！',
				);
			} else {
				var spent = this.min - this.lestMin;
				return this.getTweetURL(
					'宣言より早く' + spent.toFixed(0) + '分でやったぞ！！',
				);
			}
		},
		updateLestOfMins() {
